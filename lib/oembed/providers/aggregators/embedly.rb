module OEmbed
  class Providers
    ...
    #
    # You can append your Embed.ly API key to the provider
    # so that all requests are signed
    #     OEmbed::Providers::Embedly.endpoint += "?key=#{my_embedly_key}"
    #
    # If you don't yet have an API key you'll need to
    # sign up here: http://embed.ly/pricing
    Embedly = OEmbed::Provider.new('http://api.embed.ly/1/oembed')
    # Add all known URL regexps for Embedly.
    # To update this list run `bundle exec rake oembed:update_embedly`
    YAML.load_file(
      File.join(File.dirname(__FILE__), 'embedly_urls.yml')
    ).each do |url|
      Embedly << url
    end
