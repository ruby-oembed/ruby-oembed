    ...
    class << self
      ...
      def discover_provider(url, options = {})
        ...
        if format.nil? || format == :json
          provider_endpoint ||= begin
            /<link[^>]*href=['"]*([^\s'"]+)['"]*[^>]*application\/json\+oembed[^>]*>/.match(res)[1]
          rescue
            nil
          end
          provider_endpoint ||= begin
            /<link[^>]*application\/json\+oembed[^>]*href=['"]*([^\s'"]+)['"]*[^>]*>/.match(res)[1]
          rescue
            nil
          end
        ...
        if format.nil? || format == :xml
          ...
          provider_endpoint ||= begin
            /<link[^>]*(application|text)\/xml\+oembed[^>]*href=['"]*([^\s'"]+)['"]*[^>]*>/.match(res)[2]
          rescue
            nil
          end
