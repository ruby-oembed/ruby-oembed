...
describe OEmbed::Provider do
  ...
  let(:default) { OEmbed::Formatter.default }
  let(:provider) { OEmbed::Provider.new('http://foo.com/oembed') }
  let(:flickr) do
    p = OEmbed::Provider.new('http://www.flickr.com/services/oembed/')
    p << 'http://*.flickr.com/*'
    p
  end
  let(:qik) do
    p = OEmbed::Provider.new('http://qik.com/api/oembed.{format}', :xml)
    p << 'http://qik.com/video/*'
    p << 'http://qik.com/*'
    p
  end
  let(:viddler) do
    p = OEmbed::Provider.new('http://lab.viddler.com/services/oembed/', :json)
    p << 'http://*.viddler.com/*'
    p
  end
  ...
  describe '.new' do
    it 'should raise an ArgumentError given an invalid endpoint' do
      [
  ...
  describe '<<' do
    ...
    it 'should NOT allow multiple domain wildcards in a String URI schema',
       :pending => true do
    ...
    it 'should NOT allow a scheme wildcard in a String URI schema',
       :pending => true do
  ...
  it 'should not allow random formats to be parsed' do
    yaml_provider = OEmbed::Provider.new(
      'http://foo.com/api/oembed.{format}', :yml
    )
    yaml_provider << 'http://foo.com/*'
  ...
  describe '#build' do
    it 'should return a proper URL' do
      uri = flickr.send(:build, example_url(:flickr), :format => default)
    ...
    it 'should build correctly with query parameters in the endpoint URL' do
      ...
      expect(youtube).to include(url)

      uri = youtube.send(:build, url, :format => default)
  ...
  describe '#raw' do
    it 'should return the body on 200' do
      res = flickr.send(:raw, example_url(:flickr), :format => default)
    ...
    it 'should raise an UnknownFormat error on 501' do
      ...
      expect {
        flickr.send(
          :raw,
          File.join(example_url(:flickr), '501'),
          :format => default
        )
    ...
    it 'should raise a NotFound error on 404' do
      ...
      expect {
        flickr.send(
          :raw,
          File.join(example_url(:flickr), '404'),
          :format => default
        )
      }.to raise_error(OEmbed::NotFound)
    end
    ...
    it 'should raise an UnknownResponse error on other responses' do
      ...
      statuses_to_check = %w(405 500)

      statuses_to_check.each do |status|
        expect {
          flickr.send(
            :raw,
            File.join(example_url(:flickr), status),
            :format => default
          )
